...
2d. Subset theta values using Tajima < -1.5 & Pi < 0.001 threshold parameters:
```{r}
theta2 <- theta2[which(theta2$Tajima < -1.5 | theta2$tPsite < 0.001),]
...
write.table(outliers_PC2,
            "~/projects/eco_genomics_2025/population_genomics/mydata/ANGSD_HW1/outliers_PC2_RS",
            quote=F,
            row.names=F,
            sep="\t")
...
venndiagram_HW1 <- venn_diagram + scale_x_continuous(expand = expansion(mult = .2))

venndiagram_HW1

ggsave(filename = "venndiagram_HW1.png", plot = venndiagram_HW1)
