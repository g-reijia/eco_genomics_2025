...
```{r}
theta <- read.table("~/projects/eco_genomics_2025/population_genomics/myresults/ANGSD/diversity/2100_ALL_win50000_step50000.thetas", sep="\t", header=T)
head(theta)
```
Scale our thetas by nSites:
...
summary(theta2)
```
Histogram of how many sites are in each window that got sequenced
...
ggplot(theta2, aes(x=Chr, y=tPsite, color=Chr)) +
  ...
  ylim(0,0.07) +
  labs(title="Nucleotide diversity in 50 kb windows for population 2100", x="Scaffold", y="Theta-Pi")
```
```{r}
