...
PmGOannot <- read.table("Pmariana_GO_annotations_sorted_join.txt", header=F, fill = T)
colnames(PmGOannot)[c(1, 4)] <-c("gene", "geneID")

PmGOannot$geneID <- gsub('ID=', '', PmGOannot$geneID)
PmGOannot <- PmGOannot %>% dplyr::select(gene, geneID)
```


### YOU STOPPED HERE ###

***When testing for enrichment, R said "No enrichment can be performed - there are no feasible GO terms!" for each CoolWet treatment? Not sure what that's about
...
cwCvsH_geneList <- setNames(as.integer(cwCvsH_allgenes %in% cwCvsH_stat$geneID), cwCvsH_allgenes)
...
# Identify DEGs that have GO annotations
cwCvsDH_GO <- coolwetCvsDH_merge[coolwetCvsDH_merge$geneID %in% spruce_go$geneID, ]

# Keep only significant DEGs 
cwCvsDH_GO_sig <- subset(cwCvsDH_GO, padj < 0.05 & abs(log2FoldChange) > 1)

# Merge with GO annotations
cwCvsDH_stat <- merge(cwCvsDH_GO_sig, cwCvsDH_GO, by = "geneID")
...
cwCvsDH_geneList <- setNames(as.integer(cwCvsDH_allgenes %in% cwCvsDH_stat$geneID), cwCvsDH_allgenes)
...
```{r}
# Convert to data frame
coolwetHvsDH_df <- as.data.frame(coolwetHvsDH)

# Add geneID column from rownames
coolwetHvsDH_df$geneID <- rownames(coolwetHvsDH)
coolwetHvsDH_df <- coolwetCvsDH_df[, c("geneID", setdiff(names(coolwetHvsDH_df), "geneID"))]

coolwetHvsDH_merge <- merge(coolwetHvsDH_df, PmGOannot, by="geneID")

# Identify DEGs that have GO annotations
cwHvsDH_GO <- coolwetHvsDH_merge[coolwetHvsDH_merge$geneID %in% spruce_go$geneID, ]

# Keep only significant DEGs 
cwHvsDH_GO_sig <- subset(cwHvsDH_GO, padj < 0.05 & abs(log2FoldChange) > 1)

# Merge with GO annotations
cwHvsDH_stat <- merge(cwHvsDH_GO_sig, cwHvsDH_GO, by = "geneID")
...
cwHvsDH_geneList <- setNames(as.integer(cwHvsDH_allgenes %in% cwHvsDH_stat$geneID), cwHvsDH_allgenes)
