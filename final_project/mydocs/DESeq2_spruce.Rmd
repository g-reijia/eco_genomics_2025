...
dds <- DESeqDataSetFromMatrix(countData = countsTableRound[,day10_samps],
                              ...
                              design= ~ treatment) # + source_climate
...
pcaData <- plotPCA(vsd, intgroup=c("treatment","day_harvest"), returnData=TRUE)
percentVar <- round(100 * attr(pcaData,"percentVar"))

ggplot(pcaData, aes(PC1, PC2, color=treatment, shape=as.factor(day_harvest))) +
  geom_point(size=3) +
...
ggplot(pcaData, aes(PC1, PC2)) +
  geom_point(size=5, stroke = 1, aes(fill = treatment, shape = treatment, alpha = day_harvest)) +
  xlab(paste0("PC1: ",percentVar[1],"% variance")) +
  ylab(paste0("PC2: ",percentVar[2],"% variance")) +
  #scale_fill_manual(values=c('lightblue','lightgreen', 'purple'), labels = c("Harvest Day", "Treatment"))+
  theme_linedraw() +
...
There are no obvious clusters observed... so now I will make groups to test specific contrasts: 
```{r}
metadata_spruce$day_harvest <- as.character(metadata_spruce$day_harvest)
dds$treatment <- relevel(dds$treatment, ref = "Control")
dds$day_harvest <- relevel(dds$day_harvest, ref = "0") #getting an error here, still.

dds$group <- factor(paste0(dds$treatment, dds$day_harvest))
design(dds) <- ~ group
dds <- DESeq(dds)
resultsNames(dds)
```
