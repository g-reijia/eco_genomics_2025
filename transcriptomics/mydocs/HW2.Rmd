...
# Convert results to data frame and clean - making sure that there are no NAs in there. 
...
# You get to see the density of -log[10] with a rug plot underneath. This shows the individual genes that are along the tail distribution. In this section past the threshold/cut off line, there are about 400-500 genes.

# This is the same data as in plot B (above) which showed a histogram.
```
...
filtered_GO_results <- Gen1_res_table %>%
...
# Generate G1 plot
G1_topGO_plot <- G1_TopGO_plot(filtered_GO_results, "GO Enrichment for DEGs at Gen1")
G1_topGO_plot
...
# Keep GO terms with enough but not too many genes
filtered_GO_results <- Gen2_res_table %>%
  filter(Annotated >= 5 & Annotated < 500) %>%  # adjust as needed
  group_by(Term) %>%
  slice_min(classicFisher, n = 1) %>%  # keep only the most significant per Term
  ungroup() %>%
...
# Generate G2 plot
G2_topGO_plot <- G2_TopGO_plot(filtered_GO_results, "GO Enrichment for DEGs at Gen2")
G2_topGO_plot
...
# Keep GO terms with enough but not too many genes
filtered_GO_results <- Gen3_res_table %>%
  filter(Annotated >= 5 & Annotated < 500) %>%  # adjust as needed
  group_by(Term) %>%
  slice_min(classicFisher, n = 1) %>%  # keep only the most significant per Term
  ungroup() %>%
...
# Generate G3 plot
G3_topGO_plot <- G3_TopGO_plot(filtered_GO_results, "GO Enrichment for DEGs at Gen3")
G3_topGO_plot
...
# Keep GO terms with enough but not too many genes
filtered_GO_results <- Gen4_res_table %>%
  filter(Annotated >= 5 & Annotated < 500) %>%  # adjust as needed
  group_by(Term) %>%
  slice_min(classicFisher, n = 1) %>%  # keep only the most significant per Term
  ungroup() %>%
...
# Generate G4 plot
G4_topGO_plot <- G4_TopGO_plot(filtered_GO_results, "GO Enrichment for DEGs at Gen4")
G4_topGO_plot
```
```{r}
